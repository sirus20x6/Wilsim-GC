...
public class View implements Runnable, GLEventListener
{
    ...
    void init()
    {
        unsafeTopo = new UnsafeMemory(new byte[((340) * (263) + (263)) * 4 ]);
    ...
    public void loadModel(UnsafeMemory array)
    {
        // System.out.println("View: loadModel()\n");
       // System.out.println(unsafeTopo.getFloatAt((x+1) * y + y));

        //unsafeTopo = new UnsafeMemory(new byte[((latticeSizeX + 1) * (latticeSizeY + 1) + (latticeSizeY + 1)) * 4 ]);
        //unsafeTopo.putFloatArray(array.getFloatArray());
        //System.arraycopy(array.getFloatArray(), 0, topo, 0,((latticeSizeX + 1) * (latticeSizeY + 1) + (latticeSizeY + 1)) * 4  );

        //topo = Arrays.copyOf(array.getFloatArray(), (latticeSizeX + 1) * (latticeSizeY + 1) + (latticeSizeY + 1));
        array.unsafeCopyTo(unsafeTopo);
